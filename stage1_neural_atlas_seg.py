...
if __name__ == "__main__":
    ...
    cmd = "python preprocess_optical_flow.py --vid-path %s --gpu %s " % (vid_path, select_gpu)
    print(cmd)
    subprocess.call(cmd, shell=True)
    
    # get mask using current video
    if args.class_name == "portrait":
        cmd = "python preprocess_mask_portrait.py --vid-path %s --gpu %s " % (vid_path, select_gpu)
        print(cmd)
        subprocess.call(cmd, shell=True)
    else:
        cmd = "python preprocess_mask_rcnn.py --vid-path %s --class_name %s --gpu %s " % (vid_path, args.class_name, select_gpu)
        print(cmd)
        subprocess.call(cmd, shell=True)
